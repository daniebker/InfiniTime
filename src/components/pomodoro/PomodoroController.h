...
namespace Pinetime {
  namespace Controllers {
    ...
    public:
      enum class SessionType : uint8_t { Work, ShortBreak, LongBreak };

      enum class SessionState : uint8_t { Ready, Active, Paused, Completed };

      using TimerCallback = std::function<void()>; // <-- fix: remove (void) in favor of standard void()
      ...
      const char* GetSessionTypeIndicator() const;
      void FormatTimeRemaining(char* buffer, size_t bufferSize) const;
